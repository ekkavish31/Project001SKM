/* ============================================================
   app.jsx — shell, role/lang switch, Excel data binding, router
   ============================================================ */
          ...
          {busy ? (
            ...
            <>
              ...
              <button className="btn" style={{padding:"11px 16px",background:"#0078d4",color:"#fff"}} onClick={sharepoint}>
                <I3 name="export" size={16}/> โหลดจาก SharePoint
              </button>
            </>
          )}
          {err && <div className="note-box" style={{background:"var(--crit-soft)",color:"var(--crit)"}}><b>เกิดข้อผิดพลาด:</b> {err}</div>}
          <div className="tagline" style={{marginTop:6,lineHeight:1.6}}>
            {fsa
              ? "เบราว์เซอร์นี้รองรับการบันทึกกลับลงไฟล์เดิมได้โดยตรง (Chrome / Edge) — เหมาะกับไฟล์ที่ sync กับ SharePoint / OneDrive"
              : "เบราว์เซอร์นี้บันทึกแบบดาวน์โหลดไฟล์ใหม่ (ใช้ Chrome / Edge เพื่อเขียนทับไฟล์เดิมได้)"}
          </div>
        </div>
      </div>
    </div>
...
function App(){
  ...
  async function doSave(){
    ...
    try{
      ...
      if(r.mode==="sharepoint") showToast("บันทึกขึ้น SharePoint แล้ว ✓");
      else if(r.mode==="download") showToast("ดาวน์โหลดไฟล์ Excel แล้ว — อัปโหลดกลับ SharePoint ได้เลย");
      else if(r.mode==="file") showToast("บันทึกลงไฟล์ "+(r.name||"")+" แล้ว ✓");
      else if(r.mode==="cancel"){ /* user cancelled */ }
    }catch(e){ showToast("บันทึกไม่สำเร็จ: "+(e.message||e)); }
    setSaving(false); refresh();
  }
  async function reimport(){ try{ await window.Store.importPicker(); }catch(e){ showToast("นำเข้าไม่สำเร็จ: "+(e.message||e)); } setSelEmp(null); go("dashboard"); }
  ...
  function deleteEmp(name){ confirmDelete({ title:"ลบพนักงาน", danger:true, confirmLabel:"ลบพนักงาน",
    message:`ต้องการลบ “${name}” ออกจากระบบใช่หรือไม่? ข้อมูลการประเมินและประวัติอบรมของบุคคลนี้จะถูกลบด้วย`,
    onConfirm:()=>{ window.Store.deleteEmployee(name); setModal(null); refresh(); go(role==="employee"?"me":"people"); showToast("ลบพนักงานแล้ว"); } }); }
  function editSkill(sk){ openModal("editSkill",{skill:sk}); }
  function deleteSkill(sk){ confirmDelete({ title:"ลบทักษะ", danger:true, confirmLabel:"ลบทักษะ",
    message:`ต้องการลบทักษะ “${sk.name}” ใช่หรือไม่? คะแนนของทุกคนในทักษะนี้จะถูกลบ และการคำนวณ Gap จะปรับใหม่`,
    onConfirm:()=>{ window.Store.deleteSkill(sk.id); setModal(null); refresh(); showToast("ลบทักษะแล้ว"); } }); }
  function deleteRecord(tid){ confirmDelete({ title:"ลบบันทึกอบรม", danger:true, confirmLabel:"ลบ",
    message:"ต้องการลบรายการอบรมนี้ออกจากประวัติใช่หรือไม่?",
    onConfirm:()=>{ window.Store.deleteTraining(tid); setModal(null); refresh(); showToast("ลบรายการแล้ว"); } }); }
  ...
  const fileName = window.Store.fileName() || "—";
      ...
      <div className="app">
          ...
          <div className="sb-scope">
            <div className="scope-card">
              <div className="flex between center">
                ...
                <button onClick={reimport} title={t("changeFile")} style={{border:0,background:"transparent",color:"var(--nav-mut)",fontSize:10,fontFamily:"var(--mono)",cursor:"pointer",letterSpacing:"0.04em"}}>{t("changeFile")} ↗</button>
              </div>
        ...
        <div className="content">
          <header className="topbar">
            <span className="crumb"><b>{window.Store.current()}</b> · {crumbGroup} / <b>{crumbPage}</b></span>
